...
def get_content_from_internet(url, max_try_num=15, sleep_time=5):
    headers = {
...
def candle_minute_line(stock_code, k_type=1, num=241):
    ...
    data = get_content_from_internet(url)
...
if __name__ == '__main__':
    ...
    pool = Pool()
    pool.map(save_minute_to_file, [code for code in f_list])



    print('update_data_Costs:%.3fs' % (time.perf_counter() - start))
