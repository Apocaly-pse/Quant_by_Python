...
def import_csv(stock_code, rows=0):
    df = pd.read_csv(os.path.join(config.input_data_path, 'stock_minute_data', stock_code + '.csv'))[-rows:-241]
...
stock_code = 'sh600519'
df = import_csv(stock_code, rows=241+4820)
...
def train_model():
    ...
    gru_layer1 = GRU(64, input_shape=(time_step, 1), return_sequences=True)(input_layer)
    gru_layer2 = GRU(32, input_shape=(time_step, 64), return_sequences=False)(gru_layer1)

    dropout_layer = Dropout(.3)(gru_layer2)
...
best_model = load_model('%s_GRU_weights.07-0.0064.h5' % stock_code)
...
plt.xlabel('Index')
# plt.show()
plt.savefig('GRU_minute_%s.jpg' % stock_code, format='jpg', dpi=300)
